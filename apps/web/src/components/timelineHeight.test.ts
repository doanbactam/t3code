...
describe("estimateTimelineMessageHeight", () => {
  ...
  it("uses narrower width to increase user line wrapping", () => {
    ...
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 768 })).toBe(94);
  });
  ...
  it("does not clamp user wrapping too aggressively on very narrow layouts", () => {
    ...
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 100 })).toBe(160);
    expect(estimateTimelineMessageHeight(message, { timelineWidthPx: 320 })).toBe(94);
  });
